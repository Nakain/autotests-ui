...
def chromium_page(request: SubRequest, playwright: Playwright) -> Page:
    ...
    allure.attach.file(f"./tracing/{request.node.name}.zip", name="trace", extension="zip") #Прикрепляем трейсинг к алюр отчету
...
def chromium_page_with_state(request: SubRequest, initialize_browser_state, playwright: Playwright) -> Page:
    ...
    allure.attach.file(f"./tracing/{request.node.name}.zip", name="trace",
                       extension="zip")  # Прикрепляем трейсинг к алюр отчету
